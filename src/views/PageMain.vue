<template>
  <div class="create_webtoon_item_box">
    <input type="text" placeholder="웹툰 제목 입력"
           v-model="this.create_webtoon_title"
           @keyup.enter="onCreateWebToonItem"
    />
    <button class="create_btn" type="button"
            @click="onCreateWebToonItem"
    >추가
    </button>
  </div>
  <ul class="webtoon_list">
    <li v-for="(item, idx) in webtoon_list" :key="item.id" :data-idx="idx">
      ...
      <input
        ...
        v-model="item.회차" onclick="this.select();"/>
...
.create_webtoon_item_box{
  ...
  > input{
    display:block;
    font-size:16px;
    width:100%; height:$FORM_EL_SIZE; padding:0 10px;
  }
  
  > .create_btn{
    min-width:$FORM_EL_SIZE; width:$FORM_EL_SIZE; height:$FORM_EL_SIZE;
